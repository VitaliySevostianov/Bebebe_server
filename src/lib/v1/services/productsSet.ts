...
import fs from 'fs'
import { IProductSetCreationParams, IProductSetParams, IProductSetResponse, IProductSetsResponse } from 'lib/v1/interfaces/productsSet';
import Database from 'lib/v1/models/productsSet';
import { validate, createProductSet } from 'lib/v1/helpers/productsSet';
import { SuccessMessageResponse } from 'lib/v1/interfaces/public';

import multer from 'multer'

export class ProductsSetService {
	public async get(id: string): Promise<IProductSetResponse> {
		try {
	...
	public async getAll(page?: number,owner?: string, sort?: string, order?: string): Promise<IProductSetsResponse> {
		try {
	...
	public async update(id: string, requestBody: IProductSetParams): Promise<any> {
		try {
	...
	public async create(productCreationParams: IProductSetCreationParams): Promise<IProductSetResponse> {
		try {
			await validate(productCreationParams);
			const product = await createProductSet(productCreationParams);
			return {
				success: true,
				product,
			};
